initMap = (id, geojson, question, districtLabel) ->
  ...
  updateLegend = (map, topCategory) ->
    ...
    while i < breaks.length
      ...
      if lower < 50 and upper < 50
        ...
        div.innerHTML += "<i style=\"background:" + colors[i] + "\"></i>" + gettext("Even") + "<br/>"
      i++
  ...
  info.update = (props) ->
    ...
    if topCategory
      ...
      html += "<div class='top-border primary-color'>" + gettext("PARTICIPATION LEVEL") + "</div>"
      ...
      html += "<tr><td class='info-tiny'>" + gettext("Responses") + "</td><td class='info-tiny'>" + gettext("Reporters in") + " " + label + "</td></tr></table></div>"

      html += "<div class='top-border primary-color'>" +gettext("RESULTS") + "</div>"
      ...
      if displayOthers and results.set > 0
        ...
        html += "<div class='other-help'>" + gettext("* Other answers include:") + "</div><table>"
