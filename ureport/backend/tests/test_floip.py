...
class FLOIPBackendTest(UreportTest):
    ...
    def test_get_definition(self, mock_get):
        ...
        self.assertEqual(self.backend.get_definition(self.nigeria, "2a754346-a0dc-4176-a8b9-0f978f6b04c7"), flow_definition)

        poll = self.create_poll(self.nigeria, "Flow 1", "2a754346-a0dc-4176-a8b9-0f978f6b04c7", self.education_nigeria, self.admin)
        ...
        self.assertEqual(PollResponseCategory.objects.all().count(), 2)
    ...
    def test_pull_results(self, mock_cache_get, mock_redis_lock, mock_request):
        ...
        Contact.objects.create(org=self.nigeria, uuid='160786609', gender='M', born=1990, state='R-LAGOS',
                               district='R-OYO')
        poll = self.create_poll(self.nigeria, "Flow 1", "2a754346-a0dc-4176-a8b9-0f978f6b04c7", self.education_nigeria, self.admin)
        PollQuestion.objects.create(poll=poll, title='question 1', ruleset_uuid="q_1522956745304_75",
                                    created_by=self.admin, modified_by=self.admin)

        PollQuestion.objects.create(poll=poll, title='question 2', ruleset_uuid="q_1522956746998_26",
                                    created_by=self.admin, modified_by=self.admin)

        PollQuestion.objects.create(poll=poll, title='question 3', ruleset_uuid="q_1522957067432_34",
                                    created_by=self.admin, modified_by=self.admin)
        ...
        self.assertEqual((num_val_created, num_val_updated, num_val_ignored,
                          num_path_created, num_path_updated, num_path_ignored), (15, 0, 8, 0, 0, 0))
        mock_redis_lock.assert_called_once_with(Poll.POLL_PULL_RESULTS_TASK_LOCK % (poll.org.pk, poll.flow_uuid),
                                                timeout=7200)

        poll_result = PollResult.objects.filter(flow="2a754346-a0dc-4176-a8b9-0f978f6b04c7", ruleset='q_1522956745304_75',
                                                contact='160786609').first()
