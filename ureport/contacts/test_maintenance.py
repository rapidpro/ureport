...
class MaintenanceTest(UreportTest):
    """
...
class RebuildReportersCountsTest(MaintenanceTest):
    ...
    def _run(self, times=1):
        with patch.object(rebuild_reporters_counts, "apply_async") as mock_continue:
            for _ in range(times):
                rebuild_reporters_counts(self.job.id)
    ...
    def test_backs_off_while_a_contact_task_holds_the_lock(self, mock_recalculate):
        lock = get_valkey_connection().lock(TaskState.get_lock_key(self.nigeria, PULL_JOB_TYPE), timeout=60)
        ...
        job = SyncJob.objects.get(id=self.job.id)
        self.assertEqual(job.status, SyncJob.STATUS_COMPLETE)
        self.assertEqual(job.progress, {"lock_backoffs": 1, "chunks": 1})
    ...
    def test_reclaim_of_an_overrunning_rebuild_backs_off(self, mock_recalculate):
        ...
        lock = get_valkey_connection().lock(
            TaskState.get_lock_key(self.nigeria, PULL_JOB_TYPE), timeout=REBUILD_LOCK_TIMEOUT
        )
        self.assertTrue(lock.acquire(blocking=False))
...
class SchemesBackfillTest(MaintenanceTest):
    ...
    def test_batches_progress_across_chunks(self):
        ...
        job = SyncJob.objects.get(id=self.job.id)
        self.assertEqual(job.status, SyncJob.STATUS_RUNNING)
        self.assertEqual(job.cursor, {"max_id": self.contacts[3].id})
        self.assertEqual(job.progress, {"chunks": 2, "contacts": 4})
        ...
        job = SyncJob.objects.get(id=self.job.id)
        self.assertEqual(job.status, SyncJob.STATUS_COMPLETE)
        self.assertEqual(job.cursor, {"max_id": self.contacts[4].id})
        self.assertEqual(job.progress, {"chunks": 3, "contacts": 5})
        mock_continue.assert_not_called()
    ...
    def test_run_resumes_from_its_cursor(self):
        self._run()
        self.assertEqual(SyncJob.objects.get(id=self.job.id).cursor, {"max_id": self.contacts[1].id})
        ...
        self.assertEqual(SyncJob.objects.get(id=self.job.id).cursor, {"max_id": self.contacts[3].id})
    ...
    def test_legacy_done_flag_skips_everything(self):
        ...
        # the flag it skipped on keeps the completion time it already recorded
        self.assertEqual(cache.get(ACTIVITIES_BACKFILL.done_key % self.nigeria.id), "2026-01-01T00:00:00.000Z")
    ...
    def test_results_backfill_updates_poll_results(self):
        ...
        self._run(task=backfill_results_schemes, times=2)
...
class SchemesBackfillTriggerTest(MaintenanceTest):
    ...
    def _complete_pull(self, scope="rapidpro"):
        ...
        with patch("ureport.contacts.sync.update_cache_org_contact_counts"):
            with patch.object(backfill_activities_schemes, "apply_async") as mock_backfill:
                finalize_contacts_sync(SyncJob.objects.get(id=job.id))
    ...
    def test_trigger_forces_a_repull_the_backfill_then_waits_on(self):
        ...
        self.assertEqual(SyncJob.objects.get(id=self.pull_job.id).cursor, {})
    ...
    def test_trigger_skips_the_repull_once_schemes_are_populated(self):
        ...
        mock_pull.assert_not_called()
        self.assertEqual(SyncJob.objects.get(id=self.pull_job.id).cursor, {"last_until": "2026-08-10T10:00:00.000Z"})
        self.assertIsNone(cache.get(PENDING_KEY % self.nigeria.id))
    ...
    def test_trigger_retries_until_every_backend_is_reset(self):
        self.pull_job.claim("worker-1", lease_seconds=600)
        ...
        self.assertEqual(SyncJob.objects.get(id=self.pull_job.id).cursor["until"], "2026-08-11T10:00:00.000Z")
    ...
    def test_reset_never_clobbers_a_claimed_job(self):
        ...
        self.pull_job.claim("worker-1", lease_seconds=600)
        self.pull_job.checkpoint(cursor={"stage": "contacts", "since": None, "until": "2026-08-11T10:00:00.000Z"})
        SyncJob.objects.filter(id=self.pull_job.id).update(
            lease_expires_on=timezone.now() - timedelta(seconds=1),
            modified_on=timezone.now() - STALE_RUN_AFTER - timedelta(minutes=1),
        )
        cache.set(self.last_fetched_key, "2026-08-10T10:00:00.000Z", None)
