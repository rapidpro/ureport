...
class JobSourceTest(UreportJobsTest):
    ...
    def test_create_job_source(self):
        ...
        self.assertEqual(set(["source_type", "loc"]), set(response.context["form"].fields))
        ...
        self.assertEqual(set(["is_featured", "title", "source_url", "loc"]), set(response.context["form"].fields))
        ...
        self.assertEqual(
            set(["is_featured", "title", "source_url", "widget_id", "loc"]), set(response.context["form"].fields)
        )

        response = self.client.post(
            create_url + "?source_type=" + JobSource.FACEBOOK, dict(), SERVER_NAME="uganda.ureport.io"
        )
        ...
        response = self.client.post(
            create_url + "?source_type=" + JobSource.FACEBOOK, post_data, follow=True, SERVER_NAME="uganda.ureport.io"
        )
    ...
    def test_update_job_source(self):
        ...
        self.assertEqual(len(response.context["form"].fields), 5)
        self.assertEqual(
            set(["is_featured", "title", "source_url", "loc", "is_active"]), set(response.context["form"].fields)
        )
        ...
        self.assertEqual(
            set(["is_featured", "title", "source_url", "widget_id", "loc", "is_active"]),
