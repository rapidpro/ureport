...
class Poll(SmartModel):
    ...
    def delete_poll_results_counter(self):
        ...
        logger.info("Deleted %d poll results counters for poll #%d on org #%d" % (counters_ids_count, self.pk, self.org_id))
    ...
    def rebuild_poll_results_counts(self):
        ...
        if r.get(key):
            ...
            with r.lock(key):
                ...
                logger.info("Rebuild counts progress... build counters dict for pair %s, %s, processed %d of %d in %ds" % (org_id, flow, processed_results, poll_results_ids_count, time.time() - start))
                ...
                logger.info("Finished Rebuilding the counters for poll #%d on org #%d in %ds, inserted %d counters objects for %s results" % (poll_id, org_id, time.time() - start, len(counters_to_insert), poll_results_ids_count))

                start_update_cache = time.time()
                self.update_questions_results_cache()
                logger.info("Calculated questions results and updated the cache for poll #%d on org #%d in %ds" % (poll_id, org_id, time.time() - start_update_cache))

                logger.info("Poll responses counts for poll #%d on org #%d are %s responded out of %s polled" % (poll_id, org_id, self.responded_runs(), self.runs()))
...
class PollResult(models.Model):
    ...
    def generate_counters(self):
        ...
        if category or (self.category is not None and self.category.lower() not in PollResponseCategory.IGNORED_CATEGORY_RULES and text):
