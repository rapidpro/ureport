...
LEASE_SECONDS = {
    RESULTS_JOB_TYPE: RESULTS_LEASE_SECONDS,
    ARCHIVES_JOB_TYPE: ARCHIVES_LEASE_SECONDS,
    COUNTS_JOB_TYPE: MAINTENANCE_LEASE_SECONDS,
    PRUNE_JOB_TYPE: MAINTENANCE_LEASE_SECONDS,
    AGE_GENDER_JOB_TYPE: MAINTENANCE_LEASE_SECONDS,
}
...
def queue_counts_rebuild():
    """
    Ensures the install wide counts rebuild job exists and asks for a chunk of it to run.
    """
    job = SyncJob.get_or_create_job(None, COUNTS_JOB_TYPE)
    _enqueue(rebuild_poll_counts, job, queue=SLOW_QUEUE)
    return job


def queue_results_prune(org):
    """
    Ensures this org's old results prune job exists and asks for a chunk of it to run.
    """
    job = SyncJob.get_or_create_job(org, PRUNE_JOB_TYPE)
    _enqueue(prune_poll_results, job, queue=SLOW_QUEUE)
    return job


def queue_age_gender_backfill(org):
    """
    Ensures this org's age and gender backfill job exists and asks for a chunk of it to run.
    """
    job = SyncJob.get_or_create_job(org, AGE_GENDER_JOB_TYPE)
    _enqueue(backfill_age_gender, job, queue=SLOW_QUEUE)
    return job
...
def _enqueue(task, job, queue=SYNC_QUEUE):
...
def _is_due(job, interval, now):
    """
    A job is due unless it is paused, a run is still moving, its failure streak has earned
    a wait, or its last run started recently enough for the cadence asked for - an interval
    of None meaning any finished run is stale. The failure backoff matters because a job
    that fails deterministically would otherwise be retried on every pass.
    """
    ...
    if job.consecutive_failures:
        ...
        # the backoff is a wait after the failure, so it runs from when the run ended
        if job.ended_on and job.ended_on > now - backoff:
            return False

    if not interval:
        return True

    # the cadence runs from when the last run started, not when it ended: measured from the
    # end, a pass that takes longer than the gap between passes pushes every following one
    # out by its own duration, and a daily job that takes hours settles into two day gaps
    last_start = job.started_on or job.ended_on

    return not (last_start and last_start > now - interval)
...
def _in_flight(job, now):
    ...
    lease_seconds = LEASE_SECONDS.get(job.job_type, DEFAULT_LEASE_SECONDS)
...
def _is_fresh_run(job):
    """
    Whether this chunk is the first of a new run, i.e. the claim reset the run's counters
    rather than resuming an interrupted one. A traversal that has to cover everything on
    every run uses this to drop the position the last completed run left behind, instead
    of resuming below it and walking nothing.
    """
    return not job.progress


def _run_cursor(job):
    return {} if _is_fresh_run(job) else dict(job.cursor or {})
...
def rebuild_poll_counts(job):
    ...
    for poll in polls:
        ...
        job.checkpoint(
            cursor={"after_pk": poll.pk},
            progress=job.add_progress(rebuilt=1),
...
def prune_poll_results(job):
    ...
    for poll in polls:
        ...
        job.checkpoint(
            cursor={"after_pk": poll.pk},
            progress=job.add_progress(**{outcome: 1}),
            lease_seconds=MAINTENANCE_LEASE_SECONDS,
        )
...
def _clear_poll_results(org, poll):
    ...
    # still exist in - a chunked sync announces itself with its job lease instead. It is
    # never waited on: this chunk holds a job lease that a two hour wait would outlive
    r = get_valkey_connection()
    lock = r.lock(Poll.POLL_PULL_RESULTS_TASK_LOCK % (org.pk, poll.flow_uuid), timeout=Poll.POLL_SYNC_LOCK_TIMEOUT)
    ...
    try:
        ...
        try:
            ...
            # the poll outlived the lock's timeout - don't let that fail an otherwise
            # successful chunk or mask an in-flight exception
            logger.warning("Unable to release the poll sync lock for poll #%d as it is no longer owned" % poll.pk)
...
def backfill_age_gender(job):
    ...
    if _is_fresh_run(job):
        cursor = {"stage": STAGE_POPULATE, "after_contact_id": _populate_start(cursor)}

    # every chunk records itself before doing any work: it makes a new run's start position
    # durable, and means no chunk after the first can be taken for the start of a run
    job.checkpoint(cursor=cursor, progress=job.add_progress(chunks=1), lease_seconds=MAINTENANCE_LEASE_SECONDS)
    ...
    for poll in polls:
        ...
        job.checkpoint(
            cursor={**cursor, "stage": STAGE_REBUILD, "after_pk": poll.pk},
            progress=job.add_progress(rebuilt=1),
            lease_seconds=MAINTENANCE_LEASE_SECONDS,
        )
...
def _populate_start(cursor):
    """
    Where a new run's populate pass starts: the position this job's last run reached, or
    for a job that has never run, the position the unchunked task left in its cache key.
    That key is only ever read - the walk it belongs to is retired.
    """
    from ureport.utils import LAST_POPULATED_CONTACT_KEY

    return cursor.get("populate_watermark") or cache.get(LAST_POPULATED_CONTACT_KEY, 0) or 0
...
def _populate_chunk(job, org, cursor):
    ...
    for _ in range(POPULATE_BATCHES_PER_CHUNK):
        ...
        if not contact_ids:
            ...
            job.checkpoint(
                cursor={"stage": STAGE_REBUILD, "populate_watermark": after_contact_id},
                lease_seconds=MAINTENANCE_LEASE_SECONDS,
            )
            return False
...
def rebuild_counts_dispatch():
    ...
    if not _is_due(job, MAINTENANCE_INTERVAL, timezone.now()):
...
def prune_results_dispatch(org_id=None):
    ...
    for org in orgs:
        job = SyncJob.get_or_create_job(org, PRUNE_JOB_TYPE)
        if _is_due(job, MAINTENANCE_INTERVAL, now):
            _enqueue(prune_poll_results, job, queue=SLOW_QUEUE)
