...
BEGIN
  ...
  IF _contact.org_id IS NOT NULL AND _contact.is_active THEN
    ...
    IF _contact.registered_on IS NOT NULL THEN
      ...
      IF _contact.gender IS NOT NULL THEN
        PERFORM ureport_insert_reporters_counter(_contact.org_id, CONCAT('registered_gender:', DATE(date_trunc('month', _contact.registered_on)::timestamp), ':', LOWER(_contact.gender)), _count);
      END IF;
      IF _contact.born IS NOT NULL THEN
        PERFORM ureport_insert_reporters_counter(_contact.org_id, CONCAT('registered_born:', DATE(date_trunc('month', _contact.registered_on)::timestamp), ':', LOWER(CAST(_contact.born AS VARCHAR ))), _count);
      END IF;
      IF _contact.state IS NOT NULL THEN
        PERFORM ureport_insert_reporters_counter(_contact.org_id, CONCAT('registered_state:', DATE(date_trunc('month', _contact.registered_on)::timestamp), ':', UPPER(_contact.state)), _count);
      END IF;
...
BEGIN
  ...
  ELSIF _new_contact.org_id = _old_contact.org_id THEN
    ...
    IF _new_contact.registered_on != _old_contact.registered_on THEN
      ...
      IF _new_contact.gender != _old_contact.gender THEN
        PERFORM ureport_insert_reporters_counter(_old_contact.org_id, CONCAT('registered_gender:', DATE(date_trunc('month', _old_contact.registered_on)::timestamp), ':', LOWER(_old_contact.gender)), -1);
        PERFORM ureport_insert_reporters_counter(_new_contact.org_id, CONCAT('registered_gender:', DATE(date_trunc('month', _new_contact.registered_on)::timestamp), ':', LOWER(_new_contact.gender)), 1);
      END IF;

      IF _new_contact.born != _old_contact.born THEN
        PERFORM ureport_insert_reporters_counter(_old_contact.org_id, CONCAT('registered_born:', DATE(date_trunc('month', _old_contact.registered_on)::timestamp), ':', LOWER(CAST(_old_contact.born AS VARCHAR ))), -1);
        PERFORM ureport_insert_reporters_counter(_new_contact.org_id, CONCAT('registered_born:', DATE(date_trunc('month', _new_contact.registered_on)::timestamp), ':', LOWER(CAST(_new_contact.born AS VARCHAR ))), 1);
      END IF;

      IF _new_contact.state != _old_contact.state THEN
        PERFORM ureport_insert_reporters_counter(_old_contact.org_id, CONCAT('registered_state:', DATE(date_trunc('month', _old_contact.registered_on)::timestamp), ':', UPPER(_old_contact.state)), -1);
        PERFORM ureport_insert_reporters_counter(_new_contact.org_id, CONCAT('registered_state:', DATE(date_trunc('month', _new_contact.registered_on)::timestamp), ':', UPPER(_new_contact.state)), 1);
      END IF;
    END IF;
    IF _new_contact.state != _old_contact.state THEN
