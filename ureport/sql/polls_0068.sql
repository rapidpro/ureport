...
BEGIN
    ...
    UPDATE stats_contactactivity SET born = _poll_result.born, gender = _poll_result.gender, state = _poll_result.state, district = _poll_result.district, ward = _poll_result.ward, scheme = _poll_result.scheme WHERE org_id = _poll_result.org_id and contact = _poll_result.contact and date > date_trunc('month', CURRENT_DATE) - INTERVAL '1 year';
END;
$$ LANGUAGE plpgsql;
